...
function get_liked_review($userId){
    ...
    $query = "SELECT * FROM Review where id in (SELECT review_id from User_like_dislike where user_id = ? and type = 1) ORDER BY id DESC LIMIT 3";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "i", $userId);
    if (mysqli_stmt_execute($stmt)){
        //echo "executed";
...
function get_liked_review_view_more($userId, $cnt_post){
    include 'connection.php';
    require_once('work_around_func.php');
...
function get_disliked_review($userId){
    include 'connection.php';
    require_once('work_around_func.php');

    $query = "SELECT * FROM Review where id in (SELECT review_id from User_like_dislike where user_id = ? and type = 0)  ORDER BY id DESC LIMIT 3";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "i", $userId);
    if (mysqli_stmt_execute($stmt)){
        //echo "executed";
...
function get_disliked_review_view_more($userId, $cnt_post){
    include 'connection.php';
    require_once('work_around_func.php');
...
function get_subscribe_review($userId){
    include 'connection.php';
    require_once('work_around_func.php');
    $query = "SELECT * FROM Review WHERE user_id in (SELECT sub_to_id FROM User_subscribes WHERE user_id = ?) ORDER BY id DESC limit 3";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "i", $userId);
    if (mysqli_stmt_execute($stmt)){
        //echo "executed";
...
function get_subscribe_review_view_more($userId, $cnt_post){
    ...
    $query = "SELECT * FROM (SELECT * FROM Review WHERE user_id in (SELECT sub_to_id FROM User_subscribes WHERE user_id = ?) ORDER BY id DESC) AVA LIMIT ?,3"; // get 3 more reviews
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "ii", $userId, $cnt_post);
    if (mysqli_stmt_execute($stmt)){
        //echo "executed";
...
function check_sub($posterId, $subscriberId){
    include 'connection.php';
    require_once('work_around_func.php');
    $find_query = "SELECT COUNT(*) as cntSub FROM User_subscribes WHERE user_id = ? and sub_to_id = ?";
...
function check_like_dislike($reviewId, $userId){
    include 'connection.php';
    require_once('work_around_func.php');
    $find_query = "SELECT type FROM User_like_dislike WHERE user_id = ? and review_id = ?";
    $find_stmt = mysqli_prepare($conn, $find_query);
    mysqli_stmt_bind_param($find_stmt, "ii", $userId, $reviewId);
    if(mysqli_stmt_execute($find_stmt)){
        $result = get_result($find_stmt);
        if(count($result) > 0){
            $type = $result[0]['type'];
...
function get_number_of_subscribers($userId){
    include 'connection.php';
    require_once('work_around_func.php');
    $query = "SELECT COUNT(*) as cntSub FROM(SELECT distinct user_id FROM User_subscribes WHERE sub_to_id = ?) ali";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "i", $userId);
    if(mysqli_stmt_execute($stmt)){
        $result = get_result($stmt);
